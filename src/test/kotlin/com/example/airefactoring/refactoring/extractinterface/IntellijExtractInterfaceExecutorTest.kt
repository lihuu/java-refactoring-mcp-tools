...
class IntellijExtractInterfaceExecutorTest : LightJavaCodeInsightFixtureTestCase() {
    private val resolver = ExtractInterfaceSelectionResolver()
    private val executor = IntellijExtractInterfaceExecutor()
    ...
    fun testExtractsAndReportsFacts() {
        ...
        val prep = prepare(sourceFile, listOf("doIt"), "ServiceApi", "example.api")
        ...
        assertEquals("ServiceApi", result.interfaceName)
        assertEquals("example.api.ServiceApi", result.qualifiedInterfaceName)
        ...
        assertTrue(result.affectedFiles!!.any { it.contains("ServiceApi.java") })
        // Verify new interface content
        val newClass = JavaPsiFacade.getInstance(project).findClass("example.api.ServiceApi", GlobalSearchScope.allScope(project))
        ...
        assertTrue(afterClass.implementsList?.referencedTypes?.any { it.canonicalText == "example.api.ServiceApi" } == true)
    }

    fun testPersistsSourceAndNewFile() {
        val (sourceFile, _) = fixture()
        val prep = prepare(sourceFile, listOf("doIt"), "ServiceApi2", null)
        val persister = RecordingNativeRefactoringDocumentPersister()
        val result = runExecutor { execWithNoDialog { IntellijExtractInterfaceExecutor(persister).extract(project, prep) } }
    ...
    fun testOneUndoRestoresFiles() {
        ...
        val beforeQualified = "example.api.ServiceApi3"
        ...
        val prep = prepare(sourceFile, listOf("doIt"), "ServiceApi3", "example.api")
    ...
    private fun prepare(file: PsiJavaFile, memberNames: List<String>, interfaceName: String, targetPackage: String?): ExtractInterfacePreparation {
        com.intellij.testFramework.IndexingTestUtil.waitUntilIndexesAreReady(project)
        ...
        val res = resolver.resolve(
            ...
            interfaceName, targetPackage,
        )
        assertTrue("expected success but was $res", res is ExtractInterfaceSelectionResolution.Success)
        return (res as ExtractInterfaceSelectionResolution.Success).preparation
    }
