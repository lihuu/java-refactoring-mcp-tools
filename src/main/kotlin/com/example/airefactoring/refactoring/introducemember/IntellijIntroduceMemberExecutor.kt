 ...
 */
class IntellijIntroduceMemberExecutor : IntroduceMemberExecutor {
    ...
    override suspend fun introduce(
        ...
        val settings = BaseExpressionToFieldHandler.Settings(
            ...
            false,
            BaseExpressionToFieldHandler.TargetDestination(selection.containingClass),
            false,
            false,
        )
        ...
        writeAction {
            ...
            try {
                ...
                CommandProcessor.getInstance().executeCommand(
                    ...
                    Runnable {
                        ...
                        runNativeHandler(project, editor, selection.expression, settings, profile)
                    },
                    profile.commandName,
                    null,
                )
            ...
            val created = locateCreatedField(selection.containingClass, actualName)
                ?: throw IntroduceMemberPreparationException(
                    "Native ${profile.operationName} did not create the member '$actualName'.",
                )
    ...
    private fun runNativeHandler(
        ...
        when (profile) {
            IntroduceMemberProfile.Constant -> try {
                FixedIntroduceConstantHandler(settings).invoke(project, editor, expression)
            ...
            IntroduceMemberProfile.InstanceFinalField -> {
                val accepted = try {
                    FixedIntroduceFieldHandler(settings).run(project, editor, expression)
                } catch (e: CommonRefactoringUtil.RefactoringErrorHintException) {
                    throw preparationFrom(e, profile)
                }
                if (!accepted) {
    ...
    private fun locateCreatedField(containingClass: PsiClass, expectedName: String): PsiField? =
        containingClass.fields.singleOrNull { it.name == expectedName }
    ...
    private fun summaryFor(profile: IntroduceMemberProfile, name: String): String =
        if (profile is IntroduceMemberProfile.Constant) {
            "Introduced constant '$name'."
        } else {
            "Introduced field '$name'."
        }
}
