 ...
 *
 * The owning handler's identity and label are not stored here — the action binds a handler and its
 * target together, so read [RefactoringHandler.id] / [RefactoringHandler.displayName] from that
 * handler instead of duplicating them on every target.
 */
