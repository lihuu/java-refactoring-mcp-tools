...
class ExtractInterfaceSelectionResolver(
    ...
    fun resolve(
        ...
        val trimmedName = interfaceName.trim()
        if (trimmedName.isEmpty()) {
            return failure(McpRefactoringErrorCode.INVALID_FIELD_NAME, "Interface name must not be empty.")
        }
        if (!PsiNameHelper.getInstance(project).isIdentifier(trimmedName)) {
            return failure(McpRefactoringErrorCode.INVALID_FIELD_NAME, "Interface name '$trimmedName' is not a valid Java identifier.")
        }
        // targetPackage validation
        ...
        if (sourceClass is com.intellij.psi.PsiAnonymousClass) {
            return failure(McpRefactoringErrorCode.UNSUPPORTED_TARGET, "Anonymous classes cannot be source for Extract Interface.")
        }
        ...
        for (i in 0 until n) {
            ...
            if (containing !== sourceClass) {
                // also check via qualified name fallback for psi equality across instances? but reference equality is expected for same file resolution
                ...
                // if qualified names equal but instances differ, still consider same class if same file? For safety, compare by element equality via manager
                // Use areElementsEquivalent check
                val equivalent = com.intellij.psi.PsiManager.getInstance(project).areElementsEquivalent(containing, sourceClass)
                if (!equivalent) {
                    return failure(McpRefactoringErrorCode.UNSUPPORTED_TARGET, "All members must belong to the source class.")
                }
            }
            // V1 applicability: only public instance methods and public static final fields
            when (member) {
                is PsiMethod -> {
                    if (member.isConstructor) {
                        return failure(McpRefactoringErrorCode.UNSUPPORTED_TARGET, "Constructors cannot be extracted to interface.")
                    }
        ...
        // Resolve project-relative path from source file
        ...
        return ExtractInterfaceSelectionResolution.Success(
            ExtractInterfacePreparation(
    ...
    private fun failure(code: McpRefactoringErrorCode, message: String): ExtractInterfaceSelectionResolution.Failure =
        ExtractInterfaceSelectionResolution.Failure(code, message)
}
